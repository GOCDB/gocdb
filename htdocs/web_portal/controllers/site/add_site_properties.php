...
function add_site_properties() {
    ...
    if($_POST) {     	// If we receive a POST request it's for new properties
        ...
        $site = \Factory::getSiteService()->getSite($_REQUEST['PARENT']);
        ...
        if(isset($_REQUEST['UserConfirmed'])) {
            submit($site, $user, $propertyArray, $preventOverwrite);
        }
        else {
            confirm($propertyArray, $site, $user);
        }
 ...
 * @throws Exception
 */
function submit( \Site $site, \User $user = null, array $propArr, $preventOverwrite) {

    $serv = \Factory::getSiteService();
    $sp = $serv->addProperties($site, $user, $propArr, $preventOverwrite);
 ...
 * @param \User|null $user
 */
function confirm(array $propArr, \Site $site, \User $user = null){
...
function draw(\User $user = null) {
    ...
    $serv = \Factory::getSiteService();
    $site = $serv->getSite($_REQUEST['parentid']); //get service by id
