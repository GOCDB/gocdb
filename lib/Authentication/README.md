...
implementations to support different authentication mechanisms such as X.509,
...
* Without modification, the framework is configured for X.509 client certificate authentication.
...
the required `IFirewallComponent.php` instance.
  * Multiple  `IFirewallComponent.php` instances can be configured for different
    ...
    because credentials could be provided in every request (e.g. via X.509 or BASIC).  
