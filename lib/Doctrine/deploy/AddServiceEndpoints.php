...
if (count($scopes) !== 1) {
    throw new Exception(count($scopes) . " scopes found with " .
        "short name: EGI");
}
...
foreach ($ses as $xmlSe) {
    ...
    if (count($parentSites) !== 1) {
        throw new Exception(count($parentSites) . " sites found " .
            "with short name: " . $xmlSe->SITENAME .
            ". SE hostname is " . $xmlSe->HOSTNAME);
    }
     ...
     * one SERVICE_TYPE */
    if (count($sts) !== 1) {
        throw new Exception(count($sts) . " SERVICE_TYPEs found " .
            "with name: " . $xmlSe->SERVICE_TYPE);
    }
    ...
    $doctrineEndpointLocation->setInterfaceName((string)$doctrineSe
