...
class Site extends AbstractEntityService{
     ...
     *
     * @param \org\gocdb\services\RoleActionAuthorisationService
     *        $roleActionAuthService
     */
    public function setRoleActionAuthorisationService(
        RoleActionAuthorisationService $roleActionAuthService
    ) {
     ...
     *
     * @return Site $site The updated site entity
     */
    function editSite(\Site $site, $newValues, \User $user = null)
    {
        ...
        if (
            (
                $this->roleActionAuthorisationService
                    ->authoriseAction(
                        \Action::EDIT_OBJECT,
                        $site,
                        $user
            ...
            throw new Exception(
                "You don't have permission over " . $site->getShortName()
            );
        }
        ...
        $selectedScopesToApply = $this->scopeService
            ->getScopes($scopeIdsToApply);

        /**
         * If not admin, Check user edits to the site's Reserved scopes:
         *
         * Required to prevent users manually
         * crafting a POST request in an attempt to select reserved scopes,
         * this is unlikely but it is a possible hack.
         *
         * Site can only have reserved tags that are already assigned
         * or assigned to the parent.
         */
        if (!$user->isAdmin()) {
            $selectedReservedScopes = $this->scopeService
                ->getScopesFilterByParams(
                    array('excludeNonReserved' => true),
                    $selectedScopesToApply
                );

            $existingReservedScopes = $this->scopeService
                ->getScopesFilterByParams(
                    array('excludeNonReserved' => true),
                    $site->getScopes()->toArray()
                );

            $existingReservedScopesParent = $this->scopeService
                ->getScopesFilterByParams(
                    array('excludeNonReserved' => true),
                    $site->getNgi()->getScopes()->toArray()
                );

            foreach ($selectedReservedScopes as $sc) {
                // Reserved scopes must already be assigned to site or parent
                if (
                    !in_array($sc, $existingReservedScopes)
                    && !in_array($sc, $existingReservedScopesParent)
                ) {
                    throw new Exception(
                        "A reserved Scope Tag was selected that is "
        ...
        try {
            ...
            /**
             * Remove reserved scopes from child
             * services that are not applied on site
             */
            $siteReservedScopes = $this->scopeService
                ->getScopesFilterByParams(
                    array('excludeNonReserved' => true),
                    $site->getScopes()->toArray()
                );
            ...
            foreach ($site->getServices() as $service) {
                $serviceReservedScopes = $this->scopeService
                    ->getScopesFilterByParams(
                        array('excludeNonReserved' => true),
            ...
            } elseif ($newValues['childServiceScopeAction'] == 'inherit') {
            ...
            } elseif ($newValues['childServiceScopeAction'] == 'override') {
    ...
    private function validate($siteData, $type)
    {
        ...
        /**
         * Apply additional logic for validation that can't
         * be captured solely using gocdb_schema.xml
         */
     ...
     *   'certification_status' => String certification
     *                             status value e.g. 'Certified'
     *   'exclude_certification_status' => String exclude
     *                                     sites with this certification status
     ...
     *   'extensions' => String extensions expression
     *                   to filter custom key=value pairs
     *
     ...
     *
     * Sites are matched using SQL 'like'
     * statements so you can for e.g. specify
     * $ngiName = '%_ngi' to select all Sites whose parent
     *             NGI's name ends in '_ngi'.
     * All parametes are nullable. Null params are
     * not used for filtering results.
     *
     * @param string $ngiName NGI name
     * @param string $prodStatus Production status/target infrastructure,
     *                           usually Test or Production
     * @param string $certStatus Certification status value (Certified,
     *                           Uncertified, Candidate, Suspended, Closed)
    ...
    public function getSitesBy(
        ...
        $showClosed = null,
        $siteId = null,
        $siteExtPropKeyName = null,
        $siteExtPropKeyValue = null,
        $apiAuthId = null
    ) {
    ...
    public function getProperties($id)
    {
        ...
        $properties = $this->em->createQuery($dql)
            ->setParameter(
                'ID',
                $id
            )->getOneOrNullResult();
    ...
    public function getProperty($id)
    {
        ...
        $property = $this->em->createQuery($dql)
            ->setParameter(
                'ID',
                $id
            )->getOneOrNullResult();
    ...
    public function getPropertyByKeyAndParent($key, $parentSite)
    {
        ...
        $dql = "SELECT p FROM SiteProperty p WHERE p.keyName = :KEY "
                . "AND p.parentSite = :PARENTSITEID";
        $property = $this->em
            ->createQuery($dql)
     ...
     *
     *  @return production status with sepcified name,
     *                     throws error if multiple results
     */
     ...
     * @param integer $dayLimit Limit to downtimes that
     *                          are only $dayLimit old (can be null)
     */
    ...
    public function addSite($values, \User $user = null)
    {
        ...
        if (is_null($user)) {
            throw new Exception(
                "Unregistered users may not add new sites"
            );
        }
        ...
        $parentNgi = $this->em
            ->createQuery(
                "SELECT n FROM NGI n WHERE n.id = :id"
            )->setParameter(
                'id',
                $values['NGI']
            )->getSingleResult();
        ...
        if (!$user->isAdmin()) {
            ...
            if (
                ...
                throw new Exception(
                    ...
                    . "site to the selected NGI. "
                    . "To add a new site you require "
                    . "a managing role over an NGI"
                );
            }
        }
        ...
        $selectedScopesToApply = $this->scopeService
            ->getScopes($allSelectedScopeIds);

        /**
         * If not admin, check that requested reserved scopes
         * are already implemented by the parent NGI.
         *
         * Required to prevent users manually crafting
         * a POST request in an attempt to select reserved scopes,
         * this is unlikely but it is a possible hack.
         */
        if (!$user->isAdmin()) {
            $selectedReservedScopes = $this->scopeService
            ...
            foreach ($selectedReservedScopes as $sc) {
                ...
                if (!in_array($sc, $existingReservedScopesParent)) {
                    throw new Exception(
                        "A reserved Scope Tag was selected "
                        . "that is not assigned to the Parent NGI"
                    );
                }
            }
        }
        ...
        try {
            ...
            $this->em->persist($pk);
            /**
             * flush synchronizes the in-memory state
             * of managed objects with the database
             */
            ...
            // get the cert status
            if (
                !isset($values['Certification_Status'])
                || $values['Certification_Status'] == null
                || $values['Certification_Status'] == ''
            ) {
                throw new \LogicException(
                    "Missing seed data - No certification "
                    . "status values in the DB (required data)"
                );
            }
    ...
    private function uniqueCheck($shortName)
    {
        ...
        if (count($query->getResult()) > 0) {
            throw new Exception(
                "A site named " . $shortName . " already exists."
            );
        }
    }
    ...
    {
        $minumNumberOfScopes = $this->configService
            ->getMinimumScopesRequired('site');

        if (sizeof($myArray) < $minumNumberOfScopes) {
            throw new Exception(
                "A site must have at least "
                . $minumNumberOfScopes . " optional scope(s) assigned to it."
            );
        }
    }
     ...
     * @param $logSiteAndServicesInArchive Archive the site
     *                                     and its services or not.
     *
     * Useful for testing - an incomplete site or its service can easily
     *                      cause errors when archiving.
     *
     * @throws Exception
     */
    public function deleteSite(
        \Site $s,
        \User $user = null,
        $logSiteAndServicesInArchive = true
    ) {
        ...
        try {
            ...
            foreach ($s->getServices() as $service) {
                ...
                /**
                 * remove the service (and any downtimes
                 * associated with it and only it)
                 */
    ...
    /**
     * This method will check that a user has edit permissions
     * over a site before allowing a user to add, edit or delete
     * a site property.
     *
     * @param \User $user
     * @param \Site $site
    ...
    */
    public function userCanEditSite(\User $user, \Site $site)
    {
        ...
        if (
            $this->roleActionAuthorisationService->authoriseAction(
                \Action::EDIT_OBJECT,
                $site,
                $user
            )->getGrantAction() == true
    ...
    /**
     * Adds sets of extension property key/value pairs to a site,
     * following a request through the API
     *
     * @param \Site $site
     * @param array $propArr
     * @param bool $preventOverwrite
     ...
     * @throws \Exception
     */
    public function addPropertiesAPI(
        \Site $site,
        array $propKVArr,
        $preventOverwrite,
        $authenticationType,
        ...
        /**
         * Convert the property array into the format
         * used by the webportal logic
         */
        /**
         * @TODO
         * Make the web portal use a more sensible format
         * (e.g. array(key=> value), rather than
         * array([1]=>key,array[2]=>value))
         */
        $propArr = array();
     ...
     * @throws Exception
     */
    protected function addPropertiesLogic(
        \Site $site,
        array $propArr,
        $preventOverwrite = false
    ) {
         ...
         * and ensure we do not exceed the specified limit
         */
        $propertyCount = sizeof($existingProperties);

        foreach ($propArr as $prop) {
            /**
             * Trim off trailing and leading whitspace - as we currently
             * don't want this.
             * The input array is awkwardly formatted as keys didn't use to
             * have to be unique.
             */
            $key = trim($prop[0]);
            $value = trim($prop[1]);
            ...
            * we are preventing overwrites, this will be a problem.
            * If we are not, we will want to edit the existing property later,
            ...
            */
             ...
             * If it exists and we are preventing overwrites,
            ...
            if (is_null($property)) {
                ...
                /**
                 * Increment the property counter to enable
                 * check against property limit
                 */
                $propertyCount++;
            } elseif (!$preventOverwrite) {
                $this->editSitePropertyLogic(
                    $site,
                    $property,
                    array(
                ...
                throw new Exception(
                    "A property with name \"$key\" already exists for this "
                    . "object, no properties were added."
                );
            }
        ...
        if (count(array_unique($keys)) != count($keys)) {
            throw new Exception(
                "Property names should be unique. The requested new "
                . "properties include multiple properties with the same name."
            );
        }

        /**
         * Check to see if adding the new properties will exceed the
         * max limit defined in local_info.xml, and throw an exception if so
         */
        $extensionLimit = $this->configService->getExtensionsLimit();
        if ($propertyCount > $extensionLimit) {
            throw new Exception(
                "Property(s) could not be added due to the property limit of "
                . "$extensionLimit"
            );
        }
    }
    ...
    /**
     * Deletes site properties: validates the user has permission then calls
     * the required logic
     *
     * @param \Site $site
     * @param \User $user
     * @param array $propArr
     */
    public function deleteSiteProperties(
        \Site $site,
        \User $user,
        array $propArr
    ) {
    ...
    /**
     * Deletes site properties: validates the user
     * has permission then calls the required logic
     *
     * @param \Site $site
     * @param \User $user
     * @param array $propArr
     */
    public function deleteSitePropertiesAPI(
        \Site $site,
        array $propArr,
        $authIdentifierType,
        $authIdentifier
        ...
        foreach ($propArr as $prop) {
            if ($prop->getParentSite() != $site) {
                throw new Exception(
                    "Internal error: property parent site and site do not "
                    . "match."
                );
            }
        }
    ...
    /**
     * All the logic to delete a site's properties, before deletion a check is
     * done to confirm the property is from the parent site specified by the
     * request, and an exception is thrown if this is not the case
     *
     * @param \Site $site
     *
     * @param array $propArr
    ...
    protected function deleteSitePropertiesLogic(\Site $site, array $propArr)
    {
        foreach ($propArr as $prop) {
            ...
            if ($prop->getParentSite() != $site) {
                $id = $prop->getId();
                throw new Exception(
                    "Property {$id} does not belong to the specified site"
                );
            }
    ...
    /**
     * Edit a site's property. The user is validated then
     * the logic to make the change called
     *
     * @param \Site $site
     * @param \User $user
     ...
     * @throws \Exception
     */
    public function editSiteProperty(
        \Site $site,
        \User $user,
        \SiteProperty $prop,
        $newValues
     ...
     * A check is performed to confirm the given property is from
     * the parent site specified by the request,
     ...
     * @throws \Exception
     */
    protected function editSitePropertyLogic(
        \Site $site,
        \SiteProperty $prop,
        $newValues
    ) {
        ...
        /**
         * We don't currently want trailing
         * or leading whitespace, so we trim it
         */
        ...
        /**
         * If the properties key has changed, check there isn't an
         * existing property with that key
         */
        if ($keyname != $prop->getKeyName()) {
            ...
            foreach ($existingProperties as $existingProp) {
                if ($existingProp->getKeyName() == $keyname) {
                    throw new Exception(
                        "A property with that name already exists for this "
                        . "object"
                    );
                }
            }
    ...
    public function getScopesWithParentScopeInfo(\Site $site)
    {
        ...
        $sharedScopesNames = array_intersect(
            $childScopesNames,
            $parentScopesNames
        );

        $scopeNamesNotShared = array_diff(
            $childScopesNames,
            $parentScopesNames
        );
         ...
         * ksort($scopeNamesAndParentShareInfo, SORT_NATURAL); in php>=5.5
         */
    ...
    public function getSitesWithGeoInfo()
    {
         ...
         * We also remove sites with invaid (too large) longs and lats
         * (these are legacy values, new values entered through the webportal
         * have to be within expected range historical values may not)
         */
        $dql = "SELECT s
                FROM Site s
    ...
    /**
     * @return string xml string containing
     *                information required by the front map
     */
    public function getMapXMLString()
    {
        ...
        foreach ($sites as $site) {
            ...
            $sitePortalUrl = $portalUrl . '/index.php?Page_Type=Site&id='
                . $site->getId();

            $xmlSite->addAttribute(
                'PortalURL',
                htmlspecialchars($sitePortalUrl)
            );
            $xmlSite->addAttribute(
                'Description',
                htmlspecialchars($site->getDescription())
            );
    ...
    public function addAPIAuthEntity(\Site $site, \User $user, $newValues)
    {
        ...
        $authEnt = $authEntServ->addAPIAuthentication(
            $site,
            $user,
            $newValues
        );
    ...
    public function deleteAPIAuthEntity(
    ...
    public function editAPIAuthEntity(
        \APIAuthentication $authEntity,
        \User $user,
        $newValues
    ) {
    ...
    private function checkUserAuthz(\User $user, \Site $site)
    {
        ...
        if (!$this->userCanEditSite($user, $site)) {
            throw new Exception(
                "Permission denied: a site role is required to "
                . "modify authentication entities on site "
                . $site->getShortName()
            );
        }
    }
